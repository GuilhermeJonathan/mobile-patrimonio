...
export default function PrecoAtivoHistoricoScreen({ ticker, nome, onVoltar }: Props) {
  ...
  const [dados, setDados] = useState<PrecoAtivoHistoricoDto[]>([]);
  const [meta, setMeta] = useState<Omit<PrecoAtivoHistoricoPaginadoDto, 'items'> | null>(null);
  const [erro, setErro] = useState<string | null>(null);
  const [tooltip, setTooltip] = useState<{ x: number; y: number; item: PrecoAtivoHistoricoDto } | null>(null);
  ...
  const carregar = useCallback(async (pag: number, acumular: boolean) => {
    ...
    setErro(null);
    try {
      const res = await investimentosService.historico(ticker, pag, PAGE_SIZE);
  ...
  const valores = dadosGrafico.map(d => d.preco);
  ...
  const pontos = dadosGrafico.map((d, i) => `${toX(i)},${toY(d.preco)}`).join(' ');
  ...
  const variacaoPct = dadosGrafico.length >= 2
    ? ((dadosGrafico[dadosGrafico.length - 1].preco - dadosGrafico[0].preco) / dadosGrafico[0].preco) * 100
    : null;
  ...
  return (
    <View style={[s.root, { backgroundColor: colors.background }]}>
      ...
      <View style={s.headerInfo}>
        <Text style={[s.titulo, { color: colors.text }]}>{ticker}</Text>
        <Text style={[s.subtitulo, { color: colors.textSecondary }]}>{nome}</Text>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
              ...
              <View style={s.graficoBox}>
                ...
                <Svg width={CHART_W} height={CHART_H} style={{ alignSelf: 'center' }}>
                  ...
                  {dadosGrafico.map((d, i) => (
                    <Circle key={i} cx={toX(i)} cy={toY(d.preco)}
                      r={dadosGrafico.length <= 20 ? 4 : 2} fill={colors.green}
                      onPress={() => setTooltip(prev => prev?.item === d ? null : { x: toX(i), y: toY(d.preco), item: d })}
                    />
                  ))}
                  {tooltip && (() => {
          ...
          renderItem={({ item, index }) => {
            ...
            const diff = anterior
              ? ((item.preco - anterior.preco) / anterior.preco) * 100
              : null;
            return (
              <View style={[s.itemRow, { borderBottomColor: colors.border }]}>
                ...
                <View style={s.itemRight}>
                  <Text style={[s.itemValor, { color: colors.text }]}>R$ {numBR(item.preco, 2)}</Text>
                  {diff !== null && (
